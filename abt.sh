...
while true; do
    ...
    1)
        if check_ssh_connection; then
            rsync --dry-run --delete -avhP --info=stats2 "${KEY[@]}" $INTERNAL_ARGS "${REMOTE_HOST}":"${INTERNAL}"/ "${DESTINATION}"
            confirm_action && rsync --delete -avhP --info=stats2 "${KEY[@]}" $INTERNAL_ARGS "${REMOTE_HOST}":"${INTERNAL}"/ "${DESTINATION}"
        else
    ...
    3)
        if check_ssh_connection; then
            rsync --dry-run --delete -avhP --info=stats2 "${KEY[@]}" "${REMOTE_HOST}":"${INTERNAL}"/SwiftBackup/ "${DESTINATION}"/SwiftBackup
            confirm_action && rsync --delete -avhP --info=stats2 "${KEY[@]}" "${REMOTE_HOST}":"${INTERNAL}"/SwiftBackup/ "${DESTINATION}"/SwiftBackup
        else
            print_error_message "Failed to establish SSH connection to ${REMOTE_HOST}"
        fi
        ;;
